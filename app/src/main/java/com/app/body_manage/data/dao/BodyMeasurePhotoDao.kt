...
interface BodyMeasurePhotoDao {
    ...
    @Query(
        "SELECT " +
            "bodyMeasures.calendar_date as calendar_date," +
            "bodyMeasures.weight as weight," +
            "photos.id as photo_id," +
            "photos.photo_uri as photo_uri " +
            "from photos " +
            "INNER JOIN bodyMeasures ON bodyMeasures.ui = photos.body_measure_id " +
            "ORDER BY bodyMeasures.weight asc"
    )
    ...
    @Query(
        "SELECT " +
            ...
            "from photos " +
            "INNER JOIN bodyMeasures ON bodyMeasures.ui = photos.body_measure_id " +
            "WHERE bodyMeasures.calendar_date = :date " +
            "ORDER BY bodyMeasures.calendar_date asc"
    )
    suspend fun selectPhotosByDateOnlyDate(date: LocalDate): Map<String, List<PhotoData>>

    @Query(
        "SELECT " +
            "bodyMeasures.calendar_date " +
            "from photos " +
            "INNER JOIN bodyMeasures ON bodyMeasures.ui = photos.body_measure_id " +
            "WHERE bodyMeasures.calendar_date " +
            "BETWEEN :from AND :to " +
            "ORDER BY bodyMeasures.calendar_date asc"
    )
    ...
    @Query(
        "SELECT " +
            "bodyMeasures.calendar_date as calendar_date, " +
            "bodyMeasures.weight as weight, " +
            "bodyMeasures.fat as fat, " +
            "bodyMeasures.memo as memo, " +
            "photos.photo_uri as photo_uri " +
            "from photos " +
            "INNER JOIN bodyMeasures ON bodyMeasures.ui = photos.body_measure_id " +
